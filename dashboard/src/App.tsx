...
interface Payer { id: number; name: string; phone: string; fax: string; email: string; appealsAddress: string; avgResponseDays: number; committedResponseDays: number }
...
function App() {
  ...
  const filteredClaims = claims.filter(claim => { const matchesSearch = claimSearch === '' || claim.id.toLowerCase().includes(claimSearch.toLowerCase()) || claim.patient.toLowerCase().includes(claimSearch.toLowerCase()); const matchesPayer = claimPayerFilter === '' || claim.payer === claimPayerFilter; const matchesDate = claimDateFilter === '' || (claim.deniedDate && claim.deniedDate.startsWith(claimDateFilter)); return matchesSearch && matchesPayer && matchesDate })

  const filteredAppeals = appeals.filter(appeal => { const matchesSearch = appealSearch === '' || appeal.id.toLowerCase().includes(appealSearch.toLowerCase()) || appeal.claimId.toLowerCase().includes(appealSearch.toLowerCase()) || appeal.patient.toLowerCase().includes(appealSearch.toLowerCase()); const matchesPayer = appealPayerFilter === '' || appeal.payer === appealPayerFilter; const matchesStatus = appealFilter === 'all' || appeal.status === appealFilter; return matchesSearch && matchesPayer && matchesStatus })
      ...
      <div className="ml-64 p-8">
        ...
        {activePage === 'claims' && (
          <div className="space-y-4">
            <div className="bg-slate-800 rounded-lg p-4"><div className="flex gap-4"><div className="flex-1"><input type="text" placeholder="Search by Claim ID or Patient Name..." value={claimSearch} onChange={(e) => setClaimSearch(e.target.value)} className="w-full px-4 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none" /></div><select value={claimPayerFilter} onChange={(e) => setClaimPayerFilter(e.target.value)} className="px-4 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none"><option value="">All Payers</option>{uniquePayers.map(payer => (<option key={payer} value={payer}>{payer}</option>))}</select><input type="month" value={claimDateFilter} onChange={(e) => setClaimDateFilter(e.target.value)} className="px-4 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none" />{(claimSearch || claimPayerFilter || claimDateFilter) && (<button onClick={() => { setClaimSearch(''); setClaimPayerFilter(''); setClaimDateFilter('') }} className="px-4 py-2 text-slate-400 hover:text-white">Clear</button>)}</div>{(claimSearch || claimPayerFilter || claimDateFilter) && (<p className="text-sm text-slate-400 mt-2">Showing {filteredClaims.length} of {claims.length} claims</p>)}</div>
            <div className="bg-slate-800 rounded-lg overflow-hidden"><table className="w-full"><thead className="bg-slate-700"><tr><th className="text-left p-4 font-medium">Claim ID</th><th className="text-left p-4 font-medium">Patient</th><th className="text-left p-4 font-medium">Amount</th><th className="text-left p-4 font-medium">Payer</th><th className="text-left p-4 font-medium">Denial Reason</th><th className="text-left p-4 font-medium">Status</th><th className="text-left p-4 font-medium">Action</th></tr></thead><tbody>{filteredClaims.map((claim) => (<tr key={claim.id} className="border-t border-slate-700 hover:bg-slate-750 cursor-pointer" onClick={() => setViewingClaim(claim)}><td className="p-4 font-mono text-sm">{claim.id}</td><td className="p-4">{claim.patient}</td><td className="p-4 text-red-400">${claim.amount.toLocaleString()}</td><td className="p-4">{claim.payer}</td><td className="p-4 text-slate-400">{claim.denialReason}</td><td className="p-4"><span className={'px-2 py-1 rounded text-xs font-medium ' + getStatusStyle(claim.status)}>{claim.status}</span></td><td className="p-4">{claim.status === 'appealed' ? (<span className="text-slate-500 text-sm">Appeal submitted</span>) : (<button onClick={(e) => { e.stopPropagation(); generateAppeal(claim) }} className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 rounded text-sm">Generate Appeal</button>)}</td></tr>))}</tbody></table>{filteredClaims.length === 0 && (<div className="p-8 text-center text-slate-400">No claims found</div>)}</div>
          </div>
        )}
        ...
        {activePage === 'appeals' && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800 rounded-lg p-4"><div className="flex gap-4"><div className="flex-1"><input type="text" placeholder="Search by Appeal ID, Claim ID, or Patient Name..." value={appealSearch} onChange={(e) => setAppealSearch(e.target.value)} className="w-full px-4 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none" /></div><select value={appealPayerFilter} onChange={(e) => setAppealPayerFilter(e.target.value)} className="px-4 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none"><option value="">All Payers</option>{uniquePayers.map(payer => (<option key={payer} value={payer}>{payer}</option>))}</select>{(appealSearch || appealPayerFilter) && (<button onClick={() => { setAppealSearch(''); setAppealPayerFilter('') }} className="px-4 py-2 text-slate-400 hover:text-white">Clear</button>)}</div>{(appealSearch || appealPayerFilter || appealFilter !== 'all') && (<p className="text-sm text-slate-400 mt-2">Showing {filteredAppeals.length} of {appeals.length} appeals</p>)}</div>
        ...
        {activePage === 'analytics' && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800 rounded-lg p-6"><h3 className="text-lg font-medium mb-4">Avg Days to Decision by Payer</h3><ResponsiveContainer width="100%" height={280}><BarChart data={payers.map(p => { const pa = appeals.filter(a => a.payer === p.name && a.daysToDecision != null); const avg = pa.length > 0 ? Math.round(pa.reduce((s, a) => s + (a.daysToDecision || 0), 0) / pa.length) : 0; return { name: p.name, avgDays: avg, committed: p.committedResponseDays, appeals: pa.length } })} onClick={(data) => { if (data && data.activePayload) { const pName = data.activePayload[0].payload.name; setDrillDownData({title: pName + ' Appeals', items: appeals.filter(a => a.payer === pName)}) } }}><CartesianGrid strokeDasharray="3 3" stroke="#334155" /><XAxis dataKey="name" stroke="#94a3b8" /><YAxis stroke="#94a3b8" tickFormatter={(v) => v + 'd'} /><Tooltip contentStyle={{ backgroundColor: '#1e293b', border: 'none', borderRadius: '8px' }} formatter={(value: any, name: string) => [value + ' days', name === 'avgDays' ? 'Avg Response' : 'Committed']} /><Bar dataKey="avgDays" fill="#10b981" name="avgDays" radius={[4, 4, 0, 0]} /><Bar dataKey="committed" fill="#334155" name="committed" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer><p className="text-xs text-slate-500 mt-2 text-center">Click any bar to see that payer's appeals</p></div>
            <div className="grid grid-cols-2 gap-6">
