...
function App() {
  ...
  const filteredClaims = claims.filter(claim => { const matchesSearch = claimSearch === '' || claim.id.toLowerCase().includes(claimSearch.toLowerCase()) || claim.patient.toLowerCase().includes(claimSearch.toLowerCase()); const matchesPayer = claimPayerFilter === '' || claim.payer === claimPayerFilter; const matchesStatus = claimStatusFilter === '' || claim.status === claimStatusFilter; const tfDate = getTimeFrameDate(claimTimeFrame); const matchesTimeFrame = !tfDate || (claim.deniedDate && new Date(claim.deniedDate.split('T')[0]) >= tfDate); const matchesDate = claimDateFilter === '' || (claim.deniedDate && claim.deniedDate.startsWith(claimDateFilter)); return matchesSearch && matchesPayer && matchesStatus && matchesTimeFrame && matchesDate })

  const filteredAppeals = appeals.filter(appeal => { const matchesSearch = appealSearch === '' || appeal.id.toLowerCase().includes(appealSearch.toLowerCase()) || appeal.claimId.toLowerCase().includes(appealSearch.toLowerCase()) || appeal.patient.toLowerCase().includes(appealSearch.toLowerCase()); const matchesPayer = appealPayerFilter === '' || appeal.payer === appealPayerFilter; const matchesStatus = appealFilter === 'all' || appeal.status === appealFilter; const tfDate = getTimeFrameDate(appealTimeFrame); const matchesTimeFrame = !tfDate || (appeal.submittedDate && new Date(appeal.submittedDate.split('T')[0]) >= tfDate); return matchesSearch && matchesPayer && matchesStatus && matchesTimeFrame })
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <div className={'fixed left-0 top-0 h-full w-64 bg-slate-800 p-4 z-50 transition-transform lg:translate-x-0 ' + (sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
        ...
        <nav className="space-y-2">
          {['dashboard', 'claims', 'appeals', 'analytics'].map((page) => (<button key={page} onClick={() => { setActivePage(page); setSidebarOpen(false) }} className={'block w-full text-left px-4 py-2 rounded capitalize ' + (activePage === page ? 'bg-slate-700' : 'hover:bg-slate-700')}>{page}{page === 'appeals' && appealStats.pending > 0 && (<span className="ml-2 px-2 py-0.5 bg-blue-500 text-xs rounded-full">{appealStats.pending}</span>)}</button>))}
        </nav>
      ...
      {sidebarOpen && <div className="fixed inset-0 bg-black/50 z-40 lg:hidden" onClick={() => setSidebarOpen(false)}></div>}<div className="lg:ml-64 p-4 lg:p-8"><button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden mb-4 p-2 bg-slate-800 rounded"><span className="text-xl">☰</span></button>
        ...
        {activePage === 'dashboard' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-4">{payers.map(payer => { const payerAppeals = appeals.filter(a => a.payer === payer.name && a.daysToDecision != null); const avgDays = payerAppeals.length > 0 ? Math.round(payerAppeals.reduce((sum, a) => sum + (a.daysToDecision || 0), 0) / payerAppeals.length) : null; return (<div key={payer.id} className="bg-slate-800 rounded-lg p-4 cursor-pointer hover:bg-slate-750 transition" onClick={() => setViewingPayer(payer)}><p className="text-sm text-slate-400 mb-1">{payer.name}</p><div className="flex items-baseline gap-2">{avgDays != null ? (<><span className={'text-2xl font-bold ' + (avgDays <= payer.committedResponseDays ? 'text-emerald-400' : 'text-red-400')}>{avgDays}d</span><span className="text-slate-500 text-sm">avg to decision</span></>) : (<span className="text-slate-500">No decisions yet</span>)}</div></div>) })}
            </div>
        ...
        {activePage === 'claims' && (
          <div className="space-y-4">
            <div className="bg-slate-800 rounded-lg p-3 lg:p-4 space-y-3"><div className="flex flex-wrap gap-2 lg:gap-3"><div className="flex-1"><input type="text" placeholder="Search by Claim ID or Patient Name..." value={claimSearch} onChange={(e) => setClaimSearch(e.target.value)} className="w-full px-4 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none text-sm" /></div><select value={claimPayerFilter} onChange={(e) => setClaimPayerFilter(e.target.value)} className="px-3 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none text-sm"><option value="">All Payers</option>{uniquePayers.map(payer => (<option key={payer} value={payer}>{payer}</option>))}</select><select value={claimStatusFilter} onChange={(e) => setClaimStatusFilter(e.target.value)} className="px-3 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none text-sm"><option value="">All Statuses</option>{claimStatuses.map(s => (<option key={s} value={s}>{s}</option>))}</select><select value={claimTimeFrame} onChange={(e) => setClaimTimeFrame(e.target.value)} className="px-3 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none text-sm">{timeFrameOptions.map(tf => (<option key={tf.value} value={tf.value}>{tf.label}</option>))}</select>{(claimSearch || claimPayerFilter || claimStatusFilter || claimTimeFrame || claimDateFilter) && (<button onClick={() => { setClaimSearch(''); setClaimPayerFilter(''); setClaimStatusFilter(''); setClaimTimeFrame(''); setClaimDateFilter('') }} className="px-3 py-2 text-slate-400 hover:text-white text-sm">Clear</button>)}</div>{(claimSearch || claimPayerFilter || claimStatusFilter || claimTimeFrame || claimDateFilter) && (<p className="text-sm text-slate-400">Showing {filteredClaims.length} of {claims.length} claims</p>)}</div>
            <div className="bg-gradient-to-r from-red-600/20 to-red-600/5 border border-red-500/30 rounded-lg p-4 flex justify-between items-center"><div><p className="text-red-400 font-medium">Total Denied Amount</p><p className="text-sm text-slate-400">{filteredClaims.length} claims{(claimPayerFilter || claimStatusFilter || claimTimeFrame || claimSearch) ? ' (filtered)' : ''}</p></div><p className="text-3xl font-bold text-red-400">${filteredClaims.reduce((sum, c) => sum + c.amount, 0).toLocaleString()}</p></div>
            <div className="bg-slate-800 rounded-lg overflow-x-auto"><table className="w-full"><thead className="bg-slate-700"><tr><th className="text-left p-4 font-medium text-sm">Claim ID</th><th className="text-left p-4 font-medium text-sm">Patient</th><th className="text-left p-4 font-medium text-sm">Amount</th><th className="text-left p-4 font-medium text-sm">Payer</th><th className="text-left p-4 font-medium text-sm hidden md:table-cell">Denied Date</th><th className="text-left p-4 font-medium text-sm hidden lg:table-cell">Entered</th><th className="text-left p-4 font-medium text-sm hidden lg:table-cell">Denial Reason</th><th className="text-left p-4 font-medium text-sm">Status</th><th className="text-left p-4 font-medium text-sm">Action</th></tr></thead><tbody>{filteredClaims.map((claim) => (<tr key={claim.id} className="border-t border-slate-700 hover:bg-slate-750 cursor-pointer" onClick={() => setViewingClaim(claim)}><td className="p-2 lg:p-4 font-mono text-xs lg:text-sm">{claim.id}</td><td className="p-2 lg:p-4 text-xs lg:text-sm">{claim.patient}</td><td className="p-2 lg:p-4 text-red-400 text-xs lg:text-sm">${claim.amount.toLocaleString()}</td><td className="p-2 lg:p-4 text-xs lg:text-sm">{claim.payer}</td><td className="p-4 text-slate-400 text-sm hidden md:table-cell">{formatDate(claim.deniedDate)}</td><td className="p-4 text-slate-400 text-sm hidden lg:table-cell">{formatDate(claim.serviceDate || '')}</td><td className="p-4 text-slate-400 text-sm hidden lg:table-cell">{claim.denialReason}</td><td className="p-4"><span className={'px-2 py-1 rounded text-xs font-medium ' + getStatusStyle(claim.status)}>{claim.status}</span></td><td className="p-4">{claim.status === 'appealed' ? (<span className="text-slate-500 text-sm">Appeal submitted</span>) : (<button onClick={(e) => { e.stopPropagation(); generateAppeal(claim) }} className="px-2 lg:px-3 py-1 bg-emerald-600 hover:bg-emerald-500 rounded text-xs lg:text-sm whitespace-nowrap">Appeal</button>)}</td></tr>))}</tbody></table>{filteredClaims.length === 0 && (<div className="p-8 text-center text-slate-400">No claims found</div>)}</div>
          </div>
        )}

        {activePage === 'appeals' && (
          <div className="space-y-6">
            <div className="grid grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4">{[{ key: 'all', label: 'Total Appeals', value: appealStats.total, color: 'emerald' }, { key: 'pending', label: 'Pending', value: appealStats.pending, color: 'blue' }, { key: 'in-review', label: 'In Review', value: appealStats.inReview, color: 'yellow' }, { key: 'approved', label: 'Approved', value: appealStats.approved, color: 'emerald' }, { key: 'denied', label: 'Denied', value: appealStats.denied, color: 'red' }].map(stat => (<div key={stat.key} onClick={() => setAppealFilter(stat.key)} className={'bg-slate-800 rounded-lg p-4 cursor-pointer transition ' + (appealFilter === stat.key ? 'ring-2 ring-emerald-400' : 'hover:bg-slate-750')}><p className="text-slate-400 text-sm">{stat.label}</p><p className="text-2xl font-bold">{stat.value}</p></div>))}</div>
            <div className="bg-slate-800 rounded-lg p-3 lg:p-4 space-y-3"><div className="flex flex-wrap gap-2 lg:gap-3"><div className="flex-1"><input type="text" placeholder="Search by Appeal ID, Claim ID, or Patient Name..." value={appealSearch} onChange={(e) => setAppealSearch(e.target.value)} className="w-full px-4 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none text-sm" /></div><select value={appealPayerFilter} onChange={(e) => setAppealPayerFilter(e.target.value)} className="px-3 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none text-sm"><option value="">All Payers</option>{uniquePayers.map(payer => (<option key={payer} value={payer}>{payer}</option>))}</select><select value={appealTimeFrame} onChange={(e) => setAppealTimeFrame(e.target.value)} className="px-3 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none text-sm">{timeFrameOptions.map(tf => (<option key={tf.value} value={tf.value}>{tf.label}</option>))}</select>{(appealSearch || appealPayerFilter || appealTimeFrame || appealFilter !== 'all') && (<button onClick={() => { setAppealSearch(''); setAppealPayerFilter(''); setAppealTimeFrame(''); setAppealFilter('all') }} className="px-3 py-2 text-slate-400 hover:text-white text-sm">Clear All</button>)}</div>{(appealSearch || appealPayerFilter || appealTimeFrame || appealFilter !== 'all') && (<p className="text-sm text-slate-400">Showing {filteredAppeals.length} of {appeals.length} appeals</p>)}</div>
            <div className="bg-gradient-to-r from-emerald-600/20 to-emerald-600/5 border border-emerald-500/30 rounded-lg p-4 flex justify-between items-center"><div><p className="text-emerald-400 font-medium">Total Recovered from Appeals</p><p className="text-sm text-slate-400">From {filteredAppeals.filter(a => a.status === 'approved').length} approved appeals{(appealPayerFilter || appealTimeFrame || appealFilter !== 'all') ? ' (filtered)' : ''}</p></div><p className="text-3xl font-bold text-emerald-400">${filteredAppeals.filter(a => a.status === 'approved').reduce((sum, a) => sum + (a.recoveredAmount || 0), 0).toLocaleString()}</p></div>
            <div className="bg-slate-800 rounded-lg overflow-x-auto"><table className="w-full"><thead className="bg-slate-700"><tr><th className="text-left p-4 font-medium">Appeal ID</th><th className="text-left p-4 font-medium">Patient</th><th className="text-left p-4 font-medium hidden md:table-cell">Payer</th><th className="text-left p-4 font-medium">Amount</th><th className="text-left p-4 font-medium hidden md:table-cell">Submitted</th><th className="text-left p-4 font-medium hidden md:table-cell">Decided</th><th className="text-left p-4 font-medium hidden md:table-cell">Days</th><th className="text-left p-4 font-medium">Status</th><th className="text-left p-4 font-medium hidden lg:table-cell">Recovered</th></tr></thead><tbody>{filteredAppeals.map((appeal) => (<tr key={appeal.id} className="border-t border-slate-700 hover:bg-slate-750 cursor-pointer" onClick={() => { const claim = claims.find(c => c.id === appeal.claimId); if (claim) setViewingClaim(claim) }}><td className="p-2 lg:p-4 font-mono text-xs lg:text-sm">{appeal.id}</td><td className="p-2 lg:p-4 text-xs lg:text-sm">{appeal.patient}</td><td className="p-2 lg:p-4 text-xs lg:text-sm hidden md:table-cell">{appeal.payer}</td><td className="p-2 lg:p-4 text-xs lg:text-sm">${appeal.amount.toLocaleString()}</td><td className="p-2 lg:p-4 text-slate-400 text-xs lg:text-sm hidden md:table-cell">{formatDate(appeal.submittedDate)}</td><td className="p-2 lg:p-4 text-slate-400 text-xs lg:text-sm hidden md:table-cell">{appeal.decidedDate ? formatDate(appeal.decidedDate) : '—'}</td><td className="p-2 lg:p-4 hidden md:table-cell">{appeal.daysToDecision != null ? (<span className={appeal.daysToDecision <= 20 ? 'text-emerald-400 font-medium' : appeal.daysToDecision <= 30 ? 'text-yellow-400 font-medium' : 'text-red-400 font-medium'}>{appeal.daysToDecision}d</span>) : (<span className="text-slate-500">—</span>)}</td><td className="p-2 lg:p-4"><span className={'px-2 py-1 rounded text-xs font-medium ' + getStatusStyle(appeal.status)}>{appeal.status}</span></td><td className="p-2 lg:p-4 hidden lg:table-cell">{appeal.status === 'approved' ? (<span className="text-emerald-400 font-medium">${appeal.recoveredAmount?.toLocaleString()}</span>) : appeal.status === 'denied' ? (<span className="text-red-400">$0</span>) : (<span className="text-slate-500">—</span>)}</td></tr>))}</tbody></table>{filteredAppeals.length === 0 && (<div className="p-8 text-center text-slate-400">No appeals found</div>)}</div>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 rounded-lg max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col">
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-5">
              <div><h4 className="text-sm font-medium text-emerald-400 mb-3">Required Information</h4><div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="lg:col-span-2"><label className="block text-sm text-slate-400 mb-1">Patient Name *</label><input type="text" value={newClaim.patient} onChange={(e) => setNewClaim({ ...newClaim, patient: e.target.value })} className="w-full px-4 py-2 bg-slate-700 rounded border border-slate-600 focus:border-emerald-500 focus:outline-none" placeholder="e.g. John Smith" /></div>
